function varargout = tela_controle_com_cinematica_dir_inv(varargin)
% TELA_CONTROLE_COM_CINEMATICA_DIR_INV M-file for tela_controle_com_cinematica_dir_inv.fig
%      TELA_CONTROLE_COM_CINEMATICA_DIR_INV, by itself, creates a new TELA_CONTROLE_COM_CINEMATICA_DIR_INV or raises the existing
%      singleton*.
%
%      H = TELA_CONTROLE_COM_CINEMATICA_DIR_INV returns the handle to a new TELA_CONTROLE_COM_CINEMATICA_DIR_INV or the handle to
%      the existing singleton*.
%
%      TELA_CONTROLE_COM_CINEMATICA_DIR_INV('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TELA_CONTROLE_COM_CINEMATICA_DIR_INV.M with the given input arguments.
%
%      TELA_CONTROLE_COM_CINEMATICA_DIR_INV('Property','Value',...) creates a new TELA_CONTROLE_COM_CINEMATICA_DIR_INV or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before tela_controle_com_cinematica_dir_inv_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to tela_controle_com_cinematica_dir_inv_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help tela_controle_com_cinematica_dir_inv

% Last Modified by GUIDE v2.5 31-Oct-2017 21:59:52

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @tela_controle_com_cinematica_dir_inv_OpeningFcn, ...
                   'gui_OutputFcn',  @tela_controle_com_cinematica_dir_inv_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before tela_controle_com_cinematica_dir_inv is made visible.
function tela_controle_com_cinematica_dir_inv_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to tela_controle_com_cinematica_dir_inv (see VARARGIN)

% Choose default command line output for tela_controle_com_cinematica_dir_inv
...
%Para condizer com a notação de Denavit-Hatenberg
...
% UIWAIT makes tela_controle_com_cinematica_dir_inv wait for user response (see UIRESUME)
% uiwait(handles.figure1);



% --- Outputs from this function are returned to the command line.
function varargout = tela_controle_com_cinematica_dir_inv_OutputFcn(hObject, eventdata, handles) 
...
[angJunta, posAjustada] = angJuntas(x, y, z, gamaGraus, betaGraus, alfaGraus, handles.angMax, handles.angMin)
...
%Refaz a cinemática direta para a correção final da posição
...
if tamSeqCom > 0
    ...
    set(handles.uipSeqComandos, 'Title', [handles.titleSeqComandos ' (' filenameSeqComandos ') - Em execução']);
%     item_selected = items{index_selected};
%     display(item_selected);
      %index_selected = get(handles.listSequenciaComandos,'Value');
    for j = 1:tamSeqCom
...
[tamSeqCom,~] = size(comandos);
if tamSeqCom > 0
    set(handles.uipSeqComandos, 'Title', [handles.titleSeqComandos ' (' filenameSeqComandos ') - Continuando Execução']);
    habBotoesComando('Off', hObject, handles);
%     item_selected = items{index_selected};
%     display(item_selected);
    index_selected = get(handles.listSequenciaComandos,'Value');
...
%--- Função para interpretar e executar os comandos da lista de comandos
...
%------------------------------------------------------------------------
% Funções de Obtenção de Resposta/Feedback dos Comandos para a placa Mini
% Maestro 24
%------------------------------------------------------------------------
...
%------------------------------------------------------------------------
% Funções de Obtenção de Resposta/Feedback dos Comandos para a placa Ready
% For PIC
%------------------------------------------------------------------------
%---
...
% -----------------------------------------
% Funções de callback de conversões para os campos
% -----------------------------------------
...
%---------------------------------
% Funções para cinemática direta
%-----------------------------------

%-- Cinemática da junta 0 até a junta 4. Ângulos em graus
...
%-- Matriz para calcular a posição corrente da garra. Ângulos em graus.
%-- A posição da garra é a posição do topo de sua base.
...
%-- Função para calcular a posição da garra em X, Y, Z da base e rotações
%nestes mesmos eixos (gama, beta, alfa) em graus em notação de eixos fixos
...
%-- Função de cinemática inversa para calcular os ângulos das juntas com base nas coordenadas x,
%y, z e nas rotações em X, Y e Z (gama, beta e alfa) da garra
function [angJunta, posAjustada] = angJuntas2(x, y, z, gamaGraus, betaGraus, alfaGraus, angulosMaxGraus, angulosMinGraus)
...
posAjustada = [x, y, z, gamaGrausl, betaGrausl, alfaGrausl];

atan2pypx = atan2(py, -px);
atan2sqrt = atan2(sqrt(px2py2 - 5.415240822489),-2.327067);

teta1_1 = atan2pypx + atan2sqrt;
teta1_2 = atan2pypx - atan2sqrt;
teta1min = angulosMinGraus(1) * pi /180;
teta1max = angulosMaxGraus(1) * pi /180;

if (teta1_1 >= teta1min) && (teta1_1 <= teta1max) 
    teta1 = teta1_1;
elseif (teta1_2 >= teta1min) && (teta1_2 <= teta1max) 
    teta1 = teta1_2;
elseif teta1_1 < teta1min || teta1_2 < teta1min
    teta1 = teta1min;
elseif teta1_1 > teta1max || teta1_2 > teta1max
    teta1 = teta1max;
end
...
teta5 = atan2(s1*r11 - c1*r21, s1*r12 - c1*r22);

teta5min = angulosMinGraus(5) * pi /180;
teta5max = angulosMaxGraus(5) * pi /180;

if teta5 < teta5min
    teta5 = teta5min;
elseif teta5 > teta5max
    teta5 = teta5max;
end
...
teta3_1 = atan2(s3, c3);
teta3_2 = atan2(-s3, c3);
teta3min = angulosMinGraus(3) * pi /180;
teta3max = angulosMaxGraus(3) * pi /180;

if (teta3_1 >= teta3min) && (teta3_1 <= teta3max) 
    teta3 = teta3_1;
elseif (teta3_2 >= teta3min) && (teta3_2 <= teta3max) 
    teta3 = teta3_2;
elseif teta3_1 < teta3min || teta3_2 < teta3min
    teta3 = teta3min;
elseif teta3_1 > teta3max || teta3_2 > teta3max
    teta3 = teta3max;
end
...
teta2 = (-atan2(pz, sqrtpx2py2) - atan2(5.825*s3, 11.675+5.825*c3));

teta2min = angulosMinGraus(2) * pi /180;
teta2max = angulosMaxGraus(2) * pi /180;
if teta2 < teta2min
    teta2 = teta2min;
elseif teta2 > teta2max
    teta2 = teta2max;
end
...
teta4min = angulosMinGraus(4);
teta4max = angulosMaxGraus(4);
if teta4 < teta4min
    teta4 = teta4min;
elseif teta4 > teta4max
    teta4 = teta4max;
end
...
%-- Função de cinemática inversa para calcular os ângulos das juntas com base nas coordenadas x,
%y, z e nas rotações em X, Y e Z (gama, beta e alfa) da garra
function [angJunta, posAjustada] = angJuntas(x, y, z, gamaGraus, betaGraus, alfaGraus, angulosMaxGraus, angulosMinGraus)
